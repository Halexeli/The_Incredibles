...
TEST_SUITE("Cercle") {
    TEST_CASE("Constructeur avec paramètres") {
        ...
        CHECK(c2.diametre() == Approx(20.0)); // Diamètre = 2 * rayon
        CHECK(c2.circonference() == Approx(2 * M_PI * 10.0)); // Circonférence = 2πr
    }
    ...
    TEST_CASE("Redimensionner") {
        ...
        CHECK(c1.diametre() == Approx(16.0)); // 2 * 4 * 2
        CHECK(c1.circonference() == Approx(2 * M_PI * 8.0)); // 2π * 8
    }
}
...
TEST_SUITE("Ellipse") {
    TEST_CASE("Constructeur avec paramètres") {
        ...
        CHECK(e2.getA() == Approx(5.0));
        CHECK(e2.getB() == Approx(3.0));
        CHECK(e2.getAngle() == Approx(45.0));
        CHECK(e2.aire() == Approx(M_PI * 5.0 * 3.0)); // Aire = πab
    }
    ...
    TEST_CASE("Redimensionner avec deux facteurs") {
        ...
        CHECK(e1.getA() == Approx(10.0)); // 5 * 2
        CHECK(e1.getB() == Approx(9.0));  // 3 * 3
    }
    ...
    TEST_CASE("Pivoter") {
        ...
        Ellipse e1(centre, 5.0, 3.0, 0.0);
        e1.pivoter(45.0);
        CHECK(e1.getAngle() == Approx(45.0));
        e1.pivoter(360.0);
        CHECK(e1.getAngle() == Approx(45.0)); // Retourne à l'angle initial
        e1.pivoter(400.0);
        CHECK(e1.getAngle() == Approx(85.0)); // 45 + 400 - 360 = 85
    }

    TEST_CASE("Calcul de l'aire") {
        Point centre(0.0, 0.0);
        Ellipse e1(centre, 5.0, 3.0, 0.0);
        CHECK(e1.aire() == Approx(M_PI * 5.0 * 3.0));
    }
}
TEST_SUITE("Polyedre") {
    TEST_CASE("Constructeur avec paramètres - Polyedre virtuel") {
        Point centre(5.0, 5.0);
        Polyedre_virtuel p2(centre, 4);
        CHECK(p2.getCentre().getX() == Approx(5.0));
        CHECK(p2.getCentre().getY() == Approx(5.0));
    }

    TEST_CASE("Constructeur de copie - Polyedre virtuel") {
        Point centre(2.0, 3.0);
        Polyedre_virtuel p1(centre, 6);
        Polyedre_virtuel p2(p1);
        CHECK(p2.getCentre().getX() == p1.getCentre().getX());
        CHECK(p2.getCentre().getY() == p1.getCentre().getY());
    }
    ...
    TEST_CASE("Méthode contientPoint - Polyedre irrégulier") {
        Point centre(0.0, 0.0);
        std::stack<std::vector<std::pair<double, double>>> sommets;
        sommets.push({{0.0, 0.0}, {4.0, 0.0}, {4.0, 4.0}, {0.0, 4.0}}); // Rectangle
